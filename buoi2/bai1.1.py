...
SAVE_FOLDER = "models/no_preprocess"
...
# Clean: chỉ trim và fill missing bằng mode
...
# LabelEncode all features (baseline)
...
for name, clf in models.items():
    ...
    # Save model
    joblib.dump(clf, os.path.join(SAVE_FOLDER, f"{name}.pkl"))
...
pd.DataFrame(rows).to_csv(os.path.join(OUTPUT_DIR,'results_no_preprocess.csv'), index=False)

print("=== DONE: Model training (No Preprocessing) ===")
for name in results:
    print(f"{name}: Accuracy={results[name]['accuracy']:.4f}, F1={results[name]['f1']:.4f}")
print(f"Models saved in {SAVE_FOLDER}")
print(f"Results CSV saved in {OUTPUT_DIR}/results_no_preprocess.csv")
